...
@task(name="prepare-sv-inputs", log_prints=True)
...
@task(name="sv-from-zip", log_prints=True)
def sv_from_zip(
    zip_path: Path,
    data_dir: Path,
    *,
    ...
    """Prepare SV input NPZ from slice ZIP and extract embeddings to output NPZ."""
    zip_path = zip_path.resolve()

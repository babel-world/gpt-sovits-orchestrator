...
- **编排入口**：`uv run gpt-sovits-orchestrator <pipeline> <speaker>`
- **pipelines**：`dry-vocal`（长干声）| `ai-hobbyist`（wav+lab）
- **工作区**：`.local/{pipeline}/{speaker}/inbox` + `data/02 … data/10`

| 命令示例 | 说明 |
|----------|------|
| `uv run gpt-sovits-orchestrator dry-vocal manbo` | inbox 长音频 → data/02–09 → **subprocess 训练** → data/10 |
| `uv run gpt-sovits-orchestrator ai-hobbyist jinxi` | inbox wav+lab → data/02–09 → **subprocess 训练** → data/10 |

> **Legacy**：旧版 flat 目录 `.local/data_01` … `data_10` 不再被 orchestrator 读取；你可手动迁移或删除。
...
Prefect 共用尾链（`orchestrator_tail`）按下列顺序执行（hubert / sv / wav32k 之间无数据依赖，但代码中为串行）：
...
data_04 胶水、data_08–09 均为 orchestrator 本地处理；data_05–07 的模型推理在对应 server worker 内完成。
...
## 01 · source（dry-vocal inbox）

- **目录**：`.local/dry-vocal/{speaker}/inbox/`
- **输入**：**恰好 1 个**常见音频（`.wav` / `.mp3` / `.flac` / `.ogg` / `.m4a` / `.aac`）；不得含其它文件
- **产物**：无（inbox 即源；切片写入 `data/02`）
...
以 `dry-vocal/manbo` 为例，完整跑通后主要文件：

| 位置 | 文件 |
|------|------|
| `inbox/` | 1 条长音频（如 `manbo.mp3`） |
| `data/02` | `manbo_slices.zip` |
| `data/03` | `manbo_manifest.csv` |
| `data/04` | `manbo_manifest_g2p.csv` |
| `data/05` | `manbo_slices_hubert_in.npz`、`manbo_slices_hubert_out.npz` |
| `data/06` | `manbo_slices_sv_in.npz`、`manbo_slices_sv_out.npz` |
| `data/07` | `manbo_slices_semantic_in.npz`、`manbo_slices_semantic_out.npz` |
| `data/08` | `manbo_slices_wav32k.zip` |
| `data/09` | `manbo_manifest_assemble.csv` |
| `data/10` | `manbo.ckpt`、`manbo.pth`（trainer 产出） |
...
面向**单条长干声**：inbox 放 1 个音频 → slice → transcribe → 共用尾链。

- **编排入口**：`uv run gpt-sovits-orchestrator dry-vocal manbo`
- **Prefect flow 名**：`dry-vocal-orchestrator`

```text
.local/dry-vocal/manbo/
  inbox/                    ← 恰好 1 个音频（如 manbo.mp3）
  data/
    02/  manbo_slices.zip
    03/  manbo_manifest.csv
    …    （04–09 同 ai-hobbyist 布局）
    10/  manbo.ckpt, manbo.pth
```

### 训练

`gpt-sovits-orchestrator` 在 data/09 完成后 **subprocess** 调用 `gpt-sovits-trainer dry-vocal manbo`，权重写入 `data/10/`。  
训练超参（epoch、batch）在根 `.env` 的 `TRAINER_*`；工作区由 CLI 自动推导，无需配置 `TRAINER_WORKSPACE`。

单独重训：`cd packages/gpt-sovits-trainer && uv run gpt-sovits-trainer dry-vocal manbo`

---

## AI-Hobbyist 流水线（外部 wav + lab 数据集）

面向 [AI-Hobbyist](https://github.com/AI-Hobbyist) 等**已切片、已标注**数据集：跳过 slice 与 ASR，从 wav+lab 导入后接入 **g2p → … → assemble** 共用尾链。

- **编排入口**：`uv run gpt-sovits-orchestrator ai-hobbyist jinxi`
- **Prefect flow 名**：`ai-hobbyist-orchestrator`
- **代码包**：`src/gpt_sovits_orchestrator/ai_hobbyist/`（Python `snake_case`；磁盘路径为 `ai-hobbyist`）

```mermaid
flowchart TD
  workspace["workspace<br/>.local/ai-hobbyist/{speaker}/"]
  inbox["inbox/"]
...
为角色选一个 **ASCII speaker slug**（小写，如 `jinxi`）。将 wav+lab 拷贝到 inbox：

```text
.local/ai-hobbyist/jinxi/inbox/
...
产出示例（`speaker=jinxi`）均在 `.local/ai-hobbyist/jinxi/data/` 下：
...
与 dry-vocal 相同：orchestrator 命令会自动 subprocess 训练。单独重训：

```powershell
cd packages/gpt-sovits-trainer
uv run gpt-sovits-trainer ai-hobbyist jinxi
```
...
通常无需单独运行 trainer；`uv run gpt-sovits-orchestrator {pipeline} {speaker}` 会在 data/09 后自动 subprocess 调 trainer。
...
复制根目录 [`.env.example`](../.env.example) 为 `.env`（orchestrator 与 trainer 共用）。
...
### 验收产物（dry-vocal/manbo 示例）

| 文件 | 说明 |
|------|------|
| `data/10/manbo.ckpt` | GPT s1（底模 `s1v3.ckpt` 微调） |
| `data/10/manbo.pth` | SoVITS s2 v2Pro（底模 `s2Gv2Pro.pth` + `s2Dv2Pro.pth`） |
